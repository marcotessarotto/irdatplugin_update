...
# --- FIX: Sostituito QtWebKitWidgets con QtWebEngineWidgets ---
...
LIVELLO_LOG_COMBOBOX = {0: "Mostra tutti", 1: "Mostra livello INFO", 2: "Mostra livello WARNING",
                        3: "Mostra livello ERROR"}
...
class CatalogoIRDATDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        def write_log_message(message, tag, level):
            ...
            msg = '{time} - {tag}: {level} -  {message}'.format(
                time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), tag=tag, level=LIVELLO_LOG[level],
                message=message)
            # scrittura su file
    ...
    def message_warning(self, message):
        QMessageBox.warning(None, 'WARNING', self.tr(message))
    ...
    def message_error(self, message):
        ...
        QMessageBox.warning(None, 'ERROR', self.tr(message))
    ...
    def ricerca_elementi(self, stringa_ricerca, soggetto_ricerca, lingua):
        try:
            ...
            for layer in resp.json():
                ...
                wfs = ''
                if (tipo == 'WFS'):
                    request2_string = IRDAT_CATALOG_API + '/services/' + str(
                        id_layer) + '/connect-points?lang=' + lingua
    ...
    def ricerca_pushButton_clicked(self):
        ...
        self.message_info(
            'Ricerca e caricamento elementi nella tabella (search: ' + stringa_ricerca + ', ' + soggetto_ricerca2 + ')...')
    ...
    def carica_layer_wfs(self, layer_wfs, titolo, progettoQGIS):
        try:
            ...
            if not layerWFS.isValid():
                QMessageBox.warning(None, 'Warning', "Layer failed to load!")
            else:
